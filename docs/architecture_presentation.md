...
- `robot_control.SerialReadWriter`: pulls `SensorSample` objects from UART while exposing a command channel.
...
2. **SerialReadWriter** pushes samples to the gyro calibrator and telemetry panel simultaneously.
...
- Alternative sensor sources (e.g., file replay) can plug into `SerialReadWriter`'s interface.
- Each entry point now registers its moving pieces against the OSI layers so ROS launch files can map future nodes directly onto the same stack.
